    ...
    return (
        <div className="space-y-8">
                ...
                {classes.length > 1 && (
                    <div className="flex flex-col gap-2">
                        ...
                        <select
                            ...
                            className="bg-white border-2 border-slate-100 px-6 py-3 rounded-2xl font-bold text-slate-600 outline-none focus:border-blue-500 transition-all shadow-sm"
                        >
            ...
            {loading && children.length === 0 ? (
                <div className="flex justify-center items-center h-64 bg-white rounded-[48px] shadow-sm">
                ...
                <div className="bg-white p-16 rounded-[48px] shadow-xl border-2 border-slate-50 text-center">
                ...
                <>
                    <div className="bg-white p-2 rounded-[32px] shadow-lg border border-slate-50 flex items-center gap-2 overflow-x-auto no-scrollbar">
                        {filteredChildren.map((child: any) => (
                            <button
                                key={child.id}
                                onClick={() => setSelectedChildId(String(child.id))}
                                className={`px-8 py-4 rounded-[24px] font-black text-xs uppercase tracking-widest transition-all flex items-center gap-3 whitespace-nowrap
                                    ${selectedChildId === String(child.id)
                                        ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/20 scale-105 active:scale-95'
                                        : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
                            >
                                <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] ${selectedChildId === String(child.id) ? 'bg-white/20' : 'bg-slate-100'}`}>
                                    {child.firstName[0]}
                                </div>
                                {child.firstName} {child.lastName}
                                <span className={`ml-2 px-2 py-0.5 rounded-md text-[8px] font-bold ${selectedChildId === String(child.id) ? 'bg-white/10 text-white' : 'bg-slate-100 text-slate-400'}`}>
                                    {child.classe?.name}
                                </span>
                            </button>
                        ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                        ...
                                        <tbody className="divide-y divide-slate-50/50">
                                            {displayAttendances.length === 0 ? (
                                                ...
                                                <tr key={row.id} className="group hover:bg-slate-50/50 transition-all cursor-default">
                                                    ...
                                                    <td className="px-8 py-8">
                                                        <div className={`inline-flex items-center gap-3 px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.1em] shadow-sm
                                                            ${row.status === 'ABSENT' ? 'bg-rose-50 text-rose-500 border border-rose-100' :
                                                                row.status === 'LATE' ? 'bg-amber-50 text-amber-500 border border-amber-100' : 'bg-indigo-50 text-indigo-500 border border-indigo-100'}`}>
                                                            <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${row.status === 'ABSENT' ? 'bg-rose-500' : row.status === 'LATE' ? 'bg-amber-500' : 'bg-indigo-500'}`} />
                        ...
                        <div className="space-y-8">
                            <div className="bg-slate-900 rounded-[48px] p-10 text-white shadow-2xl relative overflow-hidden">
                                ...
                                <div className="space-y-6 relative z-10">
                                    {displayAttendances.slice(0, 3).map((alert, i) => (
                                        <div key={i} className="p-6 rounded-3xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all cursor-pointer group/alert">
                            ...
                            <button
                                onClick={downloadPDF}
                                className="w-full py-6 bg-white border-2 border-slate-100 rounded-[32px] font-black text-xs uppercase tracking-[0.2em] text-slate-900 hover:bg-slate-50 hover:border-slate-200 transition-all flex items-center justify-center gap-4 group shadow-lg"
                            >
...
const MatrixBlock = ({ label, value, color, icon: Icon }: any) => {
    ...
    return (
        <div className="bg-white p-8 rounded-[40px] shadow-xl border border-slate-50 group hover:-translate-y-1 transition-all">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 shadow-sm group-hover:scale-110 group-hover:rotate-6 transition-all ${colors[color as keyof typeof colors]}`}>
                <Icon size={28} />
            </div>
