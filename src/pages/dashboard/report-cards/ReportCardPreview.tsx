            ...
            {isOpen && (
                <div key="report-card-preview-overlay" className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 lg:p-8 print:block print:p-0 print:static">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-5xl bg-white ] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] print:max-h-none print:h-auto print: print:overflow-visible print:shadow-none print:block print:static"
                    >
                        {/* Top Toolbar - Hidden on Print */}
                        <div className="flex items-center justify-between p-6 bg-slate-50   no-print">
                            <div className="flex items-center gap-4">
                                <h3 className="text-xl font-black text-slate-800 tracking-tight">
                                    {data.length > 1 ? `Bulletins de la classe (${data.length} élèves)` : 'Aperçu du Bulletin'}
                                </h3>
                                <span className="px-3 py-1 bg-blue-100 text-blue-600  text-[10px] font-black uppercase tracking-widest">
                                    {trimester}
                                </span>
                            </div>
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-5 py-2.5 ${isSubmitting ? 'bg-slate-300' : 'bg-emerald-600 hover:bg-emerald-700'} text-white  font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-emerald-600/20 active:scale-95`}
                                >
                                    {isSubmitting ? (
                                ...
                                <button
                                    onClick={handlePrint}
                                    disabled={isPrinting}
                                    className={`flex items-center gap-2 px-5 py-2.5 ${isPrinting ? 'bg-slate-300' : 'bg-blue-600 hover:bg-blue-700'} text-white  font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-blue-600/20 active:scale-95`}
                                >
                                    <Printer size={16} /> {isPrinting ? 'Préparation...' : 'Imprimer'}
                                </button>
                                <button
                                    onClick={onClose}
                                    className="p-2.5 bg-white text-slate-400 hover:text-slate-600    transition-all font-black"
                                >
                                    <X size={20} />
                                </button>
                            </div>
                            ...
                            <div id="printable-bulletins-container" className="flex flex-col gap-10 print:gap-0 print:flex print:flex-col print:items-center print:w-full print-section">
                                {data.map((item, index) => (
                                    <div
                                        ...
                                        className={`w-full max-w-[21cm] mx-auto bg-white p-[1cm]   ] print:border-none print:shadow-none print:p-0 shadow-xl overflow-hidden print:m-0 print: report-card-printable ${index > 0 ? 'print:break-before-page mt-10 print:mt-0' : ''}`}
                                    >
                                        {/* Chadian Header */}
                                        <div className="flex justify-between items-start mb-2   pb-1">
                                            ...
                                            <div className="text-center w-1/3 flex flex-col items-center">
                                                {institution?.logoUrl ? (
                                                    ...
                                                    <div className="w-24 h-24 bg-slate-100  flex items-center justify-center mb-2">
                                        ...
                                        <div className="grid grid-cols-4 gap-2 mb-2 bg-slate-50   p-2 ">
                                            {/* Col 1: Name & ID */}
                                            <div className="  pr-2">
                                        ...
                                        {/* Main Grades Table */}
                                        <div className="  ] overflow-hidden mb-2 shadow-sm">
                                                        ...
                                                        return (
                                                            <React.Fragment key={cat}>
                                                                <tr className="bg-slate-100/80">
                                                                    <td colSpan={isEcole ? 5 : 7} className="px-4 py-1.5 text-[9px] font-black uppercase text-slate-500 tracking-widest   shadow-sm">
                                                                ...
                                                                <tr className="bg-slate-50/50">
                                                                    ...
                                                                    {!isEcole && (
                                                                        <>
                                                                            ...
                                                                            <td className="px-4 py-0.5 text-center text-[9px] font-black text-slate-600   leading-none">{totalPointsCat.toFixed(2)}</td>
                                                                        </>
                                                                    )}
                                        ...
                                        <div className="grid grid-cols-2 gap-10 mb-2 items-stretch">
                                            <div className="  ] p-6 relative overflow-hidden bg-white shadow-sm">
                                                <div className={`absolute top-0 right-0 w-24 h-24 ${getMention(Number(item.stats.generalAverage)).bg}  -mr-8 -mt-8 flex items-center justify-center pt-6 pl-6`}>
                                                    <Award size={32} className={getMention(Number(item.stats.generalAverage)).color} />
                                                </div>
                                                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2   pb-2">Résumé des résultats</h4>
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    <div className="flex justify-between items-center">
                                                        ...
                                                        <span className={`text-[10px] font-black px-3 py-1  uppercase tracking-widest ${getMention(Number(item.stats.generalAverage)).bg} ${getMention(Number(item.stats.generalAverage)).color}`}>
                                            ...
                                            <div className="  ] p-3 bg-slate-50/30 flex flex-col justify-center">
                                                {/* Historique des trimestres */}
                                                <div className="pt-2 mt-2   flex justify-between gap-1.5">
                                                    {[
                                                        { label: "Trim 1", stats: item.stats.trim1Stats },
                                                        { label: "Trim 2", stats: item.stats.trim2Stats },
                                                        { label: "Trim 3", stats: item.stats.trim3Stats }
                                                    ].map((t, i) => (
                                                        <div key={i} className={`flex-1 ${t.stats ? 'bg-white   shadow-sm' : 'bg-slate-50/50'}  p-1.5 text-center flex flex-col items-center justify-center`}>
                                                            ...
                                                            {t.stats ? (
                                                                <>
                                                                    <div className="flex flex-col items-center">
                                                                        ...
                                                                        <div className="mt-1 px-1.5 py-0.5 bg-slate-900 ">
                                                ...
                                                <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2   pb-1">{isEcole ? 'Observation de la Direction' : 'Observation du PP'}</h4>
                                        ...
                                        <div className="grid grid-cols-3 gap-8 mt-2 text-center pb-2">
                                            <div>
                                                ...
                                                <div className="w-full   mt-1 italic text-[9px] text-slate-300">Signer ici</div>
                                            </div>
                                            <div>
