    ...
    return (
        <>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                ...
                <div className="flex items-center gap-4 bg-white p-2.5 rounded-3xl shadow-sm border border-slate-100">
                    ...
                    {filterDate && (
                        <button
                            ...
                            className="text-[10px] font-black uppercase text-slate-400 hover:text-red-500 mr-3 px-2 py-1 bg-slate-50 rounded-lg border border-slate-100 transition-all"
                        >
                            Réinitialiser
                        </button>
                    )}
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-6">
                        {paginatedAttendances.map((session: any, idx: number) => (
                            <div key={idx} className="bg-white rounded-[40px] shadow-xl border border-slate-100 overflow-hidden hover:border-blue-200 transition-all group">
                                <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                    <div className="flex items-center gap-4">
                                        <div className="w-14 h-14 bg-white rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center justify-center">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="bg-rose-50 text-rose-600 px-4 py-2 rounded-2xl text-[10px] font-black tracking-widest uppercase">
                                ...
                                <div className="p-6 overflow-x-auto">
                                    <table className="w-full text-left">
                                        <thead>
                                            <tr className="text-[9px] font-black uppercase tracking-widest text-slate-300 border-b border-slate-50">
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-center gap-4 py-8">
                            <button
                                ...
                                className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${currentPage === 1 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-white text-blue-600 shadow-lg hover:scale-105 active:scale-95 border border-slate-100'}`}
                            >
                                <ChevronLeft size={16} /> Précédent
                            </button>
                            <div className="px-6 py-3 bg-white rounded-2xl border border-slate-100 shadow-sm font-black text-xs text-slate-400 tracking-[0.2em] uppercase">
                                Page <span className="text-blue-600">{currentPage}</span> / {totalPages}
                            </div>
                            <button
                                onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))}
                                disabled={currentPage === totalPages}
                                className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${currentPage === totalPages ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-white text-blue-600 shadow-lg hover:scale-105 active:scale-95 border border-slate-100'}`}
                            >
