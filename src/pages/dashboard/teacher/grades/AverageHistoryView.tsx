        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-5 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {averageHistory.map((session) => (
                    <div key={session.key || session.id} className="bg-white ] p-6 shadow-xl   hover:shadow-2xl hover: transition-all group relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-50 ] -mr-16 -mt-16 group-hover:bg-indigo-100 transition-colors"></div>

                        <div className="relative">
                            <div className="flex items-center justify-between mb-4">
                                <div className="p-3 bg-indigo-600 text-white  shadow-lg shadow-indigo-600/20">
                                    <Calculator size={20} />
                                </div>
                            ...
                            <div className="flex items-center justify-between pt-4  ">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="px-4 py-2 bg-indigo-50 text-indigo-600  text-[10px] font-black uppercase hover:bg-indigo-600 hover:text-white transition-all"
                                    >
                                        Voir / Editer
                                    </button>
                                    <button
                                        onClick={() => handlePublishSession(session)}
                                        className="p-2 bg-emerald-50 text-emerald-600  hover:bg-emerald-600 hover:text-white transition-all"
                                        title={`Envoyer ${dest}`}
                                    >
                                        <Send size={16} />
                                    </button>
                ...
                {averageHistory.length === 0 && (
                    <div className="col-span-full py-20 text-center bg-slate-50    ]">
