    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div className="lg:col-span-2 bg-white ] shadow-2xl   overflow-hidden min-h-[500px]">
                    <div className="p-8   flex items-center justify-between bg-slate-50/20">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="px-3 py-1 bg-white  text-[10px] font-black uppercase text-blue-600 tracking-widest leading-none shadow-sm">{homeworks.length} Devoirs</div>
                        </div>
                    </div>
                                        ...
                                        <div className="p-6 ] bg-slate-50/50 hover:bg-white hover:shadow-xl hover:shadow-blue-900/5   hover: transition-all duration-300 relative overflow-hidden">
                                            <div className="flex justify-between items-start mb-4">
                                                <div className="flex gap-4 items-center">
                                                    <div className={`w-12 h-12  flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform bg-blue-50 text-blue-600`}>
                                                        <BookOpen size={24} />
                                                    </div>
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            <span className="px-3 py-1 bg-blue-100 text-blue-600  text-[10px] font-black uppercase tracking-widest">{hw.subject?.name || 'Matière'}</span>
                                                            <span className={`px-3 py-1  text-[10px] font-black uppercase tracking-widest ${hw.type === 'EXAMEN' ? 'bg-rose-100 text-rose-600' : hw.type === 'TP' ? 'bg-amber-100 text-amber-600' : 'bg-indigo-100 text-indigo-600'}`}>
                                                                {hw.type || 'DEVOIR'}
                                                            </span>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                ...
                                                <span className={`px-4 py-1.5  text-[10px] font-black uppercase tracking-wider shadow-sm  ${mySubmissionsMap[hw.id]
                                                    ? (new Date(mySubmissionsMap[hw.id].submittedAt) <= new Date(hw.deadline) ? 'bg-emerald-50 text-emerald-600 ' : 'bg-amber-50 text-amber-600 ')
                                                    : (isLate ? 'bg-rose-50 text-rose-500 ' : 'bg-blue-50 text-blue-600 ')
                                                    }`}>
                                                ...
                                                {/* Correction Status */}
                                                {mySubmissionsMap[hw.id] && (
                                                    <span className={`px-4 py-1.5  text-[10px] font-black uppercase tracking-wider shadow-sm  ${mySubmissionsMap[hw.id].status === 'GRADED' && hw.gradesPublished
                                                ...
                                                <div className="flex items-center gap-1.5 px-4 py-1.5 bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none   ">
                ...
                <div className="space-y-4">
                    <div className="bg-slate-900 p-10 ] text-white shadow-2xl relative overflow-hidden flex flex-col justify-center min-h-[300px]">
            ...
            {isModalOpen && selectedHomework && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white ] w-full max-w-3xl relative z-10 shadow-3xl overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-10 bg-slate-50/50   flex justify-between items-start">
                            <div className="flex flex-col gap-2">
                                <span className="bg-blue-600 text-white text-[10px] font-black px-4 py-1.5  uppercase tracking-widest w-fit shadow-lg shadow-blue-600/20">{selectedHomework.subject?.name || 'Devoir'}</span>
                            ...
                            <button onClick={() => setIsModalOpen(false)} className="p-3 hover:bg-white  transition-all shadow-sm  "><X size={24} /></button>
                        </div>
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {((selectedHomework.attachmentUrls || selectedHomework.attachmentUrl || '') as string).split(',').filter(Boolean).map((url, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-6 bg-blue-50    group transition-all">
                                                <div className="flex items-center gap-4 truncate">
                                                    <div className="w-10 h-10 bg-white  flex items-center justify-center text-blue-600 shadow-md">
                                                        <FileText size={20} />
                                                    </div>
                                                ...
                                                <div className="flex gap-2">
                                                    <a href={getFileUrl(url)} target="_blank" rel="noreferrer" className="bg-white text-blue-600 px-4 py-2  font-black text-[10px] uppercase tracking-widest shadow-sm hover:shadow-md transition-all no-underline">Voir</a>
                                                    <a href={getFileUrl(url, true)} download className="bg-blue-600 text-white px-4 py-2  font-black text-[10px] uppercase tracking-widest shadow-sm hover:bg-blue-700 transition-all no-underline">
                                                        <Download size={14} />
                                                    </a>
                                                </div>
                                            </div>
                                ...
                                {mySubmission ? (
                                    <div className="space-y-6">
                                            ...
                                            {mySubmission.fileUrl && (
                                                <div className="flex flex-wrap gap-2 mb-4">
                                                    {mySubmission.fileUrl.split(',').filter(Boolean).map((url: string, idx: number) => (
                                                        <div key={idx} className="flex flex-col gap-2 p-4 bg-white/50    min-w-[150px]">
                                                            ...
                                                            {mySubmission.status === 'GRADED' && selectedHomework.gradesPublished && mySubmission.teacherAnnotations && (
                                                                <button
                                                                    ...
                                                                    className="w-full mt-2 py-2 bg-rose-50 hover:bg-rose-100 text-rose-600    font-black text-[10px] uppercase tracking-widest transition-all text-center flex items-center justify-center gap-2"
                                                                >
                                        ...
                                        {mySubmission.status === 'GRADED' && selectedHomework.gradesPublished && (
                                            <div className="bg-slate-900 text-white p-8 ] shadow-2xl relative overflow-hidden">
                                                ...
                                                <div className="flex justify-between items-start mb-6">
                                                    ...
                                                    <div className="w-16 h-16 bg-white/10  flex items-center justify-center backdrop-blur-md">
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-6 bg-slate-50 p-8 ]  ">
                                        <div className="space-y-3">
                                            ...
                                            <textarea
                                                ...
                                                className="w-full bg-white   ] px-8 py-6 text-sm font-medium shadow-inner focus:ring-4 focus:ring-blue-500/10 focus: transition-all outline-none"
                                            ></textarea>
                                        </div>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            {submissionFileUrls.length > 0 && (
                                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                                                    {submissionFileUrls.map((_url, idx) => (
                                                        <div key={idx} className="relative group p-3 bg-white    flex items-center justify-between">
                                            ...
                                            <div className="relative">
                                                ...
                                                <label
                                                    ...
                                                    className="flex items-center justify-center gap-3 w-full bg-white    ] px-8 py-8 cursor-pointer hover:bg-blue-50 hover: transition-all group"
                                                >
                                                    {isUploading ? (
                                        ...
                                        <button
                                            ...
                                            className="w-full bg-blue-600 text-white py-6 ] font-black shadow-2xl shadow-blue-600/30 hover:bg-blue-700 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:hover:scale-100 transition-all tracking-widest uppercase text-xs flex items-center justify-center gap-3"
                                        >
...
const LegendItem = ({ label, value, color, percent }: { label: string, value: string, color: string, percent: number }) => (
    <div className="group">
        ...
        <div className="h-2 w-full bg-white/5  overflow-hidden  ">
            <div
                className={`h-full ${color}  shadow-[0_0_15px_rgba(0,0,0,0.5)] transition-all duration-1000 ease-out`}
                style={{ width: `${percent}%` }}
            ></div>
        </div>
    </div>
